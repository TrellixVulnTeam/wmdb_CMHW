...
@search_api.route('/search/rating', methods=['POST'])
def search_rating():
    movie = request.form['movie']
    with sql.connect(db_path) as con:
        cur = con.cursor()
        query = "SELECT MOVIE.title, AVG(REVIEW.rating) as rating  FROM MOVIE, REVIEW WHERE MOVIE.MID == REVIEW.MID AND MOVIE.title LIKE ? GROUP BY MOVIE.title", (movie,)
        cur.execute(query)
        result = cur.fetchall()
